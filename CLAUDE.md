...
- **Model — `physics.ts`**：球與皮卡丘們的確定性物理引擎。直接從逆向工程出來的機器碼移植。使用整數風格的位元運算（`| 0`）以及 `rand.ts` 裡的 seeded PRNG — 在沒有充分理解的情況下，**不要**「現代化」成浮點數或 `Math.random()`，否則會與原作行為失同步。場地是 432×304；檔案開頭那一組常數是 load-bearing 的。每次 `runEngineForNextFrame()` 回傳一個 `PhysicsTickResult`（`{ isBallTouchingGround, sounds }`），其中 `sounds: SoundEvent[]` 取代了過去散在 `Player`/`Ball` 上的 mutable `sound.*` flag — engine 不再持有播放狀態，controller (`pikavolley.ts`) 拿到 array 後在 audio 層展開。引擎已從原作的 1v1 寫死，泛化成 N 玩家：`PikaPhysics.players: Player[]` 長度可以是 2（1v1）或 4（2v2），`physicsEngine` 的兩個 player loop 都跑 `players.length`。2v2 時 `processPlayerToPlayerCollisions(players)` 在每 tick 的「玩家移動」與「球-玩家碰撞」之間插入一輪，對同隊配對做 AABB 解析（垂直重疊大就疊頭頂、水平重疊大就互推 + clamp 到自己半場）。`Player.hasPlayerOnHead` / `standingOnTeammate` 兩個 flag 由這輪設置，jump 守衛變成「頭上沒人 + (在地或在隊友頭頂)」 — 1v1 時這兩個 flag 永遠 false，行為與原作完全一致。
...
- **View — `view.ts`**：PixiJS 渲染。`IntroView`、`MenuView`、`GameView`、`FadeInOut` 在 `pikavolley.ts` 裡被掛到 root stage 之下。`GameView` 構造子接 `playerCount: 2 | 4 = 2`，`playerSprites: AnimatedSprite[]` 與 `shadows.forPlayers: Sprite[]` 兩個陣列依此長度建立；`drawPlayersAndBall(physics)` 直接遍歷 `physics.players`。2v2 時 P3 / P4 套上 `tint`（黃綠 / 粉橙）做視覺區分，1v1 完全沿用原色。內部用了 `getSheetTextures` / `getTexture` 兩個小 helper 在邊界處 throw，避免在整個檔案散布 `!` 非空斷言。
- **Controller — `pikavolley.ts`**：`PikachuVolleyball` class 擁有 physics、view、audio 與一組 `PikaKeyboard`。`mode: '1v1' | '2v2'` 從 `localStorage('pv-offline-mode')` 載入（預設 1v1），決定 `keyboardArray`（`[PikaKeyboard, PikaKeyboard, ...PikaKeyboard[]]`，2 或 4 把）與 physics 的玩家數。2v2 時 P3 / P4 用 J/L/I/K/U 與 Numpad 4/6/8/2/0 預設綁定。`setMode(mode)` 在執行期切換：unsubscribe 舊鍵盤、重建 physics、重建 GameView（連帶 sprite tint 更新）、`restart()` 回 intro。menu state 的「跟電腦 / 跟朋友」在 2v2 下解釋為隊伍級別 — 跟電腦 = 按下 Z 或 Enter 那位真人 + 其他 CPU；跟朋友 = P1+P2 真人對打 + P3/P4 CPU 隊友。`scores: [number, number]` 仍是兩隊的計分（球落左半 → 右隊得分），勝利時 `for (const p of physics.players)` 把整隊的 `isWinner` / `gameEnded` 設好。以 `normalFPS = 25` 執行；回合結束後的慢動作會降到 `slowMotionFPS = 5` 持續 `SLOW_MOTION_FRAMES_NUM` 個 frame。`gameLoop()` 是由 PIXI `Ticker` 驅動的狀態機 tick。`state: GameState` 是個 function reference（`= () => void`），會在 method 之間互相重新指派（`intro` / `menu` / `round` 等）— 透過 property 呼叫 `this.state()` 時 `this` 會正確 rebind。`startRecording(seed)` / `stopRecording()` 是 opt-in 的 replay 錄製鉤子（`round()` 會在每個 physics tick 之前 push input snapshot），合約：必須在進入 `round` 前呼叫，否則 RNG 已經被消耗，replay 會對不上。對稱的 `startReplay(replay)` 會 setCustomRng + 依 `replay.playerCount` 重建 `PikaPhysics` + 把 state 直接設成 `round`，刻意跳過 `startOfNewGame` 的視覺 intro 來避免 `initializeForNewRound` 二次消耗 seed rng。多回合可 bit-perfect 重現：recording 時 `beforeStartOfNextRound` 會落 `pendingReInitMarker`，下次 `round()` push 時 flush 進 `recordingReInits` 帶進 `Replay.roundReInits`；playback 時 controller 自然會走 `beforeStartOfNextRound` 消耗那 2 個 rand，不需要額外手動補。`isRecording` / `isReplaying` getter 給 UI 用。
...
- **`replay.ts`**：把確定性引擎的「seed + 每 frame input array」打包成可序列化的 `Replay` 物件，並提供 `runReplay()` 從頭重跑、`serializeReplay` / `deserializeReplay` 做 JSON round-trip。內含 `mulberry32` 種子 PRNG（測試與 replay 共用）。`Replay.frames` 元素是 `ReplayInputFrame = readonly [Si, Si] | readonly [Si, Si, Si, Si]`（union of tuple），對應 1v1 與 2v2；optional `playerCount?: 2 | 4` 與 `isPlayer3Computer? / isPlayer4Computer?` 在 2v2 時帶上，缺值預設 `playerCount = 2`，所以 2v2 之前錄的舊 1v1 replay JSON 仍可載入並 bit-perfect 重播。多回合對局透過 optional `roundReInits: { atFrame, isPlayer2Serve }[]` 標記每個 inter-round 的 `Player.initializeForNewRound()` 點，`runReplay` 會在指定 frame 之前先對 `physics.players` 全體補上 reInit rand 消耗，避免從第 2 回合開始發散。只覆蓋 physics 層的 tick — controller 層的計分 / 慢動作 / 狀態機不在這個 module 的 scope 內。
